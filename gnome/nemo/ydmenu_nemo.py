...
class YaDiskNemoExtension(GObject.GObject, Nemo.MenuProvider):
    ...
    def _build_submenu(self, files) -> Nemo.MenuItem:
        submenu = Nemo.Menu()
        ...
        for label, act in actions:
            item = Nemo.MenuItem(name=f"YaDisk::{act}", label=f"YaDisk – {label}")
            item.connect('activate', make_activate(act))
            submenu.append_item(item)

        top = Nemo.MenuItem(name="YaDisk::Top", label="YaDisk")
