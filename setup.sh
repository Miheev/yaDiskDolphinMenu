...
output=$( awk -v line="yaDisk=\$YA_DISK_ROOT/$YA_DISK_RELATIVE;" "!x{x=sub(/^yaDisk=.*/,line)}1" ./ydmenu.sh | awk -v line="streamDir=\$yaDisk/$INBOX_RELATIVE;" "!x{x=sub(/^streamDir=.*/,line)}1" )
